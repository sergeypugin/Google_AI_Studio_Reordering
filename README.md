...
По поводу студии:
- Гугл очень хитро придумал, что каждый чат задаётся именно своим уникальным `id`!
...
> >[!warning]
> > После выполнения скрипта рекомендуется закрыть, а затем открыть волты `Obsidian`, если вдруг они содержат папку `Google AI Studio` и Вы не увидили изменений, т.к. работа с файлами этих волт осуществляется вне `Obsidian`. 
