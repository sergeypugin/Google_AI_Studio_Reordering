...
1. [До / После](#До%20/%20После)
2. [Подводные камни](#Подводные%20камни)
3. [Быстрый старт](#Быстрый%20старт)
4. [Автоматизация через `config.json`](#Автоматизация%20через%20`config.json`)
...
>[!important]
> После выполнения скрипта студия продолжить работать в обычном режиме, а значит продолжит скидывать новые чаты и аттачменты в папку `Google AI Studio` на самый верхний уровень. Благодаря тому, что все обработанные файлы и чаты лежат в папках `_Trash` и `Chats`, а также тому, что `_map.json` начинается с символа `_`, они будут появляться строго снизу (при сортировке в алфавитном порядке).
...
Почему этот формат? Потому что существует его очень удобная читалка `Obsidian`. 
...
Если вдруг Вас студия Вас забанит / перестанет работать в РФ или ещё что, то у Вас останутся все чаты и все аттачменты.
